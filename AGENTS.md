...
- Two roles are ongoing (Mintyn and Astravest). The hero's "Now" card lists every ongoing role with
  equal weight, the one marked `hero: true` (Mintyn) first. `previous` ("Before that")
